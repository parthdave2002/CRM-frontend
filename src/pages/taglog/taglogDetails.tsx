...
const TaglogDetailsPage: FC = function () {
  ...
  useEffect(() =>{
    if(id){
        // setLoading(true)
        dispatch(getTagloglist({ id : id}))   
    }
  },[id]);
  
  const { Tagloglist } = useSelector((state: any) => ({
    Tagloglist: state.PackingType.Tagloglist,
  }));

  useEffect(() => {  
    setTaglogList(Tagloglist ? Tagloglist : null);
  }, [Tagloglist]);

  let Name = "Taglog Details";
...
export default TaglogDetailsPage;